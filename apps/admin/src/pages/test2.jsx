...
const AdminBookFormPage = () => {
    ...
    if (isEditing) {
      axios.get(`/api/books/${id}/`).then((response) => {
        ...
        if (isEditedBook && book.contributions) {
          const groupedContributions = book.contributions.reduce(
            (acc, current) => {
              // Logic to strip the prefix, e.g., "Chapter 1: My Title" -> "My Title"
              // It splits by ": " and takes everything after the first part.
              const rawTitle = current.chapter_title
                .split(": ")
                .slice(1)
                .join(": ");

              const existingChapter = acc.find(
                (item) => item.chapter_title === rawTitle // Compare with raw title
              );

              if (existingChapter) {
                existingChapter.contributors.push(current.contributor.id);
              } else {
                acc.push({
                  chapter_title: rawTitle, // <-- FIX: Store ONLY the raw title in state
                  contributors: [current.contributor.id],
                });
              }
              return acc;
            },
  ...
  const handleDialogOpen = (field, index = null) => {
    ...
    if (field === "authors") {
      ...
      currentValue = contributions[index].contributors;
    }
  ...
  const handleDialogSave = () => {
    ...
    const selectedIds = tempSelection.map((opt) => opt.value);

    if (targetField === "contributors") {
      // contributions state ke andar specific chapter ko update karein
      handleContributionChange(chapterIndex, "contributors", selectedIds);
    } else {
      // 'authors' ya 'editors' ke liye formData ko update karein
      setFormData((prev) => ({ ...prev, [targetField]: selectedIds }));
    }

    handleDialogClose(); // Kaam hone ke baad dialog band kar dein
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (bookType === "authored") {
      ...
      const finalContributions = contributions.flatMap((chapter, index) => {
        ...
        // 'chapter.contributors' ab ek array of IDs hai, jise humne step 1 me banaya tha
                    ...
                    {contributions.map((c, i) => (
                      ...
                      >
                        <Grid item size={{ xs: 12, md: 4 }}>
                          <Box>
                            <Button
                              variant="outlined"
                              onClick={() =>
                                handleDialogOpen("contributors", i)
                              }
                            >
                              Select Contributor(s)
                            </Button>
                            {/* Display selected contributors' names */}
                            <Typography
                              variant="body2"
                            ...
                            >
                              {metaData.allAuthors
                                .filter((opt) =>
                                  c.contributors.includes(opt.value)
                                )
                                .map((opt) => opt.label.split(" ")[1]) // Sirf first name dikhayein
                                .join(", ") || "No contributors selected."}
                            </Typography>
                          </Box>
                        </Grid>
                        <Grid
                        ...
                        >
                          ...
                          {/* Editable text field for the RAW title */}
                          <TextField
                            fullWidth
                            sx={{ flexGrow: 1 }}
                            label="Chapter Title"
                            value={c.chapter_title} // Now this correctly binds to the raw title in the state
                            onChange={(e) =>
