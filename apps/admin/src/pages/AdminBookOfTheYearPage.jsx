...
const AdminBookOfTheYearPage = () => {
    ...
    const fetchBookOfTheYear = () => {
        ...
        axios.get('/api/books/?is_book_of_the_year=true')
            ...
            .catch(error => {
                console.error("Error fetching Book of the Year", error);
                setLoading(false);
            });
    };
    ...
    const setAsBookOfTheYear = (book) => {
        axios.patch(`/api/books/${book.id}/toggle-boty/`, { is_book_of_the_year: true })
            ...
            .catch(() => alert('Failed to set new Book of the Year.'));
    };
    ...
    const removeBookOfTheYear = (book) => {
        axios.patch(`/api/books/${book.id}/toggle-boty/`, { is_book_of_the_year: false })
            ...
            .catch(() => alert('Failed to remove Book of the Year.'));
    };

    return (
        <Box>
            <Typography variant="h4" gutterBottom>Manage Book of the Year</Typography>

            <Paper sx={{ p: 2, mb: 4 }}>
                <Typography variant="h5" gutterBottom>Current Book of the Year</Typography>
                {loading ? <CircularProgress /> : (
                    currentBook ? (
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 1, mt: 2, border: '1px solid #ddd', borderRadius: 1 }}>
                            ...
                            <Button variant="outlined" color="error" size="small" onClick={() => removeBookOfTheYear(currentBook)}>
                                Remove
                            </Button>
                        </Box>
                    ) : (
                        <Typography sx={{ mt: 2 }} color="text.secondary">No Book of the Year is currently set.</Typography>
                    )
                )}
            </Paper>

            <Paper sx={{ p: 2 }}>
                <Typography variant="h5" gutterBottom>Set a New Book of the Year</Typography>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2, maxHeight: 400, overflowY: 'auto' }}>
                    {searchResults.map(book => (
                        <Box key={book.id} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 1, border: '1px solid #ddd', borderRadius: 1 }}>
                            ...
                            <Button variant="contained" color="success" size="small" onClick={() => setAsBookOfTheYear(book)}>
                                Set as Book of the Year
                            </Button>
                        </Box>
                    ))}
                </Box>
...
export default AdminBookOfTheYearPage;